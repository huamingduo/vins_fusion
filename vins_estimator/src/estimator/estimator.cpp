...
void Estimator::Initialization() {
  if (!STEREO && USE_IMU) {  // monocular + IMU initilization
    if (frame_count == WINDOW_SIZE) {
      bool result = false;
      if (ESTIMATE_EXTRINSIC != 2 && (Headers[frame_count] - initial_timestamp) > 0.1) {
        result = initialStructure();
        initial_timestamp = Headers[frame_count];
      }
      if (result) {
        optimization();
        updateLatestStates();
  ...
  } else if (STEREO && USE_IMU) {  // stereo + IMU initilization
  ...
  } else if (STEREO && !USE_IMU) {  // stereo only initilization
    f_manager.initFramePoseByPnP(frame_count, Ps, Rs, tic, ric);
    f_manager.triangulate(frame_count, Ps, Rs, tic, ric);
    optimization();
...
void Estimator::RunTimeProcess() {
  ...
  if (failureDetection()) {  // currently always false
...
void Estimator::double2vector() {
  ...
  if (USE_IMU) {
    ...
    for (int i = 0; i <= WINDOW_SIZE; i++) {
      ...
      Ps[i] =
          rot_diff * Vector3d(para_Pose[i][0] - para_Pose[0][0], para_Pose[i][1] - para_Pose[0][1], para_Pose[i][2] - para_Pose[0][2]) + origin_P0;
...
void Estimator::slideWindow() {
  switch (marginalization_flag) {
    case MARGIN_OLD: {  // keyframe
      ...
      if (frame_count == WINDOW_SIZE) {
        ...
        if (true || solver_flag == INITIAL) {
          map<double, ImageFrame>::iterator it_0 = all_image_frame.find(t_0);
    ...
    case MARGIN_SECOND_NEW:  // non-keyframe
    default: {
      if (frame_count == WINDOW_SIZE) {
