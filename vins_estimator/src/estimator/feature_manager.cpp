...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td) {
  ...
  if (frame_count < 2 || last_track_num < 20 || long_track_num < 40 || new_feature_num > 0.5 * last_track_num) return true;
...
void FeatureManager::triangulatePoint(Eigen::Matrix<double, 3, 4> &Pose0, Eigen::Matrix<double, 3, 4> &Pose1, Eigen::Vector2d &point0,
...
void FeatureManager::removeFront(int frame_count) {
  for (auto it = feature.begin(); it != feature.end();) {
    if (it->start_frame == frame_count) {  // frame_count is actually WINDOW_SIZE
      --it->start_frame;
      it = std::next(it);
      continue;
    }
